...
#Archivo antecedente hecho por Juan Gonzales
#lectura de variables en pantalla de varioguide humana
...
import struct
size = 5
#prueba de iniciacion de funcion de camara gopro
...
import pytesseract
#se asigna direccion de libreria tesseract
pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"
#se inicia la camara que se quiere utilizar
cam = cv2.VideoCapture(1)#webcam 1
...
#empieza la aplicacion
    def run(self):
        ...
        #definicion de el frame para la aplicación
                ...
                if aspect_ratio > 2.4:
                    ...
                    def keep_box(contour):
                        ...
                        if w_ / h_ < 0.1 or w_ / h_ > 10:
                            if DEBUG == 1:
                                print("\t Rejected because of shape: (" + str(xx) + "," + str(yy) + "," + str(
                                    w_) + "," + str(h_) + ")" + str(w_ / h_))
                    ...
                    for index_, contour_ in enumerate(contours):
                        ...
                        if keep(contour_) and include_box(index_, hierarchy, contour_):
                            ...
                            if DEBUG == 0:
                                ...
                                cv2.putText(processed, str(index_), (x, y - 5), cv2.FONT_HERSHEY_PLAIN, 1,
                                            (255, 255, 255))
                        else:
                            if DEBUG == 0:
                                cv2.rectangle(rejected, (x, y), (x + w, y + h), (100, 100, 100), 1)
                                cv2.putText(rejected, str(index_), (x, y - 5), cv2.FONT_HERSHEY_PLAIN, 1,
                                            (255, 255, 255))
